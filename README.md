...
> **État au 28/08/2026 : socle local implémenté et audité.**
...
> événements sont présents. Le pack de déploiement Oracle Cloud, le Blueprint
> Render, les images de production, les sauvegardes chiffrées et la rotation
> sont maintenant
> fournis. La recette Lighthouse/axe-core, les scans CVE/ZAP, le SMTP réel et
> un test de restauration sur le VPS restent à exécuter avant la production.

---

## 1. Démarrage rapide

Déploiement hébergé recommandé : [guide Render](docs/deploiement-render.md).
Le fichier `render.yaml` provisionne le site public, Payload, PostgreSQL et le
Cron de sauvegarde. Le guide commence par la création du compte Render et
explique aussi l’utilisation des URL `onrender.com` sans domaine acheté.
...
Le rapport détaillé de l’audit local, avec preuve, sévérité, correction et
statut pour chaque domaine, est disponible dans
[`docs/rapport-audit-complet-2026-08-28.md`](docs/rapport-audit-complet-2026-08-28.md).
...
| `docker-compose.prod.yml` | Production Oracle autonome, sans override de développement ; ne publie que Nginx sur 80/443. |

Production : `docker compose --env-file .env.oracle -f docker-compose.prod.yml up -d`
